...
Highly optimized vectorized Python routines for instant multi-hundred-megabyte generation.
"""
...
def create_bmp_bytes(width: int, height: int, pattern_type: str = "gradient", bpp: int = 24) -> bytes:
    ...
    if pattern_type == "gradient":
        # Precompute 256 unique gradient rows and cycle/interpolate through them
...
def create_qoi_bytes(width: int, height: int, pattern_type: str = "gradient") -> bytes:
    ...
    # Clamp QOI generation to reasonable dimension for ultra fast generation
    eff_w = min(width, 1024)
    eff_h = min(height, 1024)
    header = struct.pack('>4sIIBB', b'qoif', eff_w, eff_h, 4, 0)
    
    bytes_out = bytearray(header)
    # Generate RLE blocks of 62 identical pixels for fast solid/gradient chunks
